// ========== ОПРЕДЕЛЕНИЕ ТЕКУЩЕГО ЯЗЫКА И СТРАНИЦЫ ==========
let currentLang = window.location.pathname.startsWith('/en/') ? 'en' : 'ru';
localStorage.setItem('trashLang', currentLang);

// Определяем страницу по последнему сегменту пути (без слешей)
let currentPage = 'home';
const pathParts = window.location.pathname.replace(/^\/[a-z]{2}\//, '').split('/').filter(p => p);
if (pathParts.length === 0) currentPage = 'home';
else currentPage = pathParts[0]; // 'top', 'rating', 'confession', 'contact'

// ========== ДАННЫЕ ДЛЯ КАРТОЧЕК (полные списки) ==========
...
function switchLanguage(lang) {
    ...
    // Перенаправляем на ту же страницу, но на другом языке
    let newPath = '/' + lang + '/';
    if (currentPage !== 'home') newPath += currentPage + '/';
...
function highlightActivePage() {
    ...
    document.querySelectorAll(".nav-menu a").forEach(link => {
        ...
        // Сравниваем путь: для главной /ru/ или /en/
        if (href === '/ru/' || href === '/en/') {
...
function updateCounterUI() {
    ...
    if (el) el.innerHTML = `${prefix}${total} | 🤮 ${sum}`;
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 5. В зависимости от страницы загружаем контент
    if (currentPage === 'home') {
        renderCards();
        // обработчики для главной
        ...
        document.getElementById("sortAZ")?.addEventListener("click", () => { sortMode = "az"; renderCards(); });
        document.getElementById("sortZA")?.addEventListener("click", () => { sortMode = "za"; renderCards(); });
        ...
        if (container) {
            ...
            container.innerHTML = facts.map((f, i) => `<div class="glass-card" style="margin-bottom:20px;"><h3>💩 Fact ${i+1}</h3><p>${f}</p></div>`).join('');
        }
    }
        ...
        function renderRating() {
            ...
            container.innerHTML = `<table style="width:100%; border-collapse: collapse;">
                <tr><th>#</th><th>Category</th><th>Votes</th><th>Action</th></tr>
                ${sorted.map((cat, idx) => `
    ...
    else if (currentPage === 'confession') {
        ...
        // статика, ничего не делаем
    }

    // Параллакс-эффект (для всех страниц)
